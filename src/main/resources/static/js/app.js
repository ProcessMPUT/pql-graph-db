...
const sampleQueries = {
    ...
    "🔹 Show First 20 Events": "select event:* limit 20",
    "🔹 Show First 10 Traces": "select trace:* limit 10",
    "🔹 Show Log Attributes": "select log:*",
    "🔹 Mixed Scope (Trace ID + Event Activity)": "select trace:caseId, event:activity limit 20",
    "🔹 Hoisting (Event -> Trace)": "select event:activity, ^event:caseId limit 20",

    // === SIMPLE FILTERS ===
    "🏥 Events from Radiology Department": "select event:activity, event:timestamp, event:org_group where event:org_group = 'Radiology' limit 30",
    "🏥 Events from General Lab": "select event:activity, event:timestamp where event:org_group = 'General Lab Clinical Chemistry' limit 25",
    "🏥 Events from Operating Rooms": "select event:activity, event:org_group where event:org_group = 'Operating rooms' limit 30",

    // === AGGREGATIONS - Show Power of PQL ===
    "📊 Count All Events": "select count(event:id)",
    "📊 Events per Department (Top 10)": "select event:org_group, count(event:id) group by event:org_group order by count(event:id) desc limit 10",
    "📊 Top 15 Most Frequent Activities": "select event:activity, count(event:id) group by event:activity order by count(event:id) desc limit 15",
    ...
    "⭐ Recent Events (Latest First)": "select event:activity, event:org_group, event:timestamp order by event:timestamp desc limit 25"
};
...
verifyButton.addEventListener('click', async () => {
    ...
    const logNameText = activeLogSelect.options[activeLogSelect.selectedIndex]?.text || logId;
    ...
    try {
        ...
        verificationResult.innerHTML = `<div style="color:red; padding: 1rem;">Error: ${error.message}</div>`;
    } finally {
