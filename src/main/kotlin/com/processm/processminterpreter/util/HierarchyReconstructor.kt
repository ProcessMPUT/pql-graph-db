...
import com.processm.processminterpreter.model.hierarchical.EventClassifier
import com.processm.processminterpreter.model.hierarchical.Extension
import com.processm.processminterpreter.model.hierarchical.GlobalAttribute
...
object HierarchyReconstructor {
    ...
    private val INTERNAL_PROPERTIES =
        setOf(
            "importOrder",
            "traceId",
            "eventId",
    ...
    fun reconstruct(
        ...
        val limitedLogGroups =
            if (logLimit != null && logLimit > 0) {
        ...
        val logs =
            limitedLogGroups.map { (logId, logRecords) ->
        ...
        record.forEach { (key, value) ->
            when {
                ...
                key in currentColumnAliases -> {
                    ...
                    val isDatetimeExtraction =
                        alias.pqlExpression.startsWith("year(") ||
                ...
                key.startsWith("l_") -> {
                    result["log"]!![key.substring(2)] = value
                }

                key.startsWith("log_") -> {
                    result["log"]!![key.substring(4)] = value
                }

                key.startsWith("t_") -> {
                    result["trace"]!![key.substring(2)] = value
                }

                key.startsWith("trace_") -> {
                    result["trace"]!![key.substring(6)] = value
                }

                key.startsWith("e_") -> {
                    result["event"]!![key.substring(2)] = value
                }

                key.startsWith("event_") -> {
                    result["event"]!![key.substring(6)] = value
                }
    ...
    private fun extractLogId(record: Map<String, Any?>): String? =
        when {
            record.containsKey("logId") -> {
                record["logId"]?.toString()
            }

            record.containsKey("l_logId") -> {
                record["l_logId"]?.toString()
            }

            record.containsKey("log_logId") -> {
                record["log_logId"]?.toString()
            }
    ...
    private fun buildLog(
        ...
        val limitedTraceGroups =
            if (traceLimit != null && traceLimit > 0) {
     ...
     */
    private fun populateLogAttributes(
        log: Log,
        ...
        copyAttributes(
            logData,
            log.attributes,
            excludeKeys = setOf("name", "logId", "lifecycle:model", "identity:id", "concept:name", "classifiers", "createdAt", "updatedAt"),
        )
        ...
        // Read globals from log data (stored as JSON by XESLoader), fallback to defaults
        val traceGlobalsJson = logData["traceGlobals"] as? String
        val eventGlobalsJson = logData["eventGlobals"] as? String
        if (traceGlobalsJson != null) {
            try {
                val traceGlobalsMap =
                ...
                traceGlobalsMap.forEach { (key, value) ->
                    log.traceGlobals.add(GlobalAttribute("trace", mapOf(key to value)))
                }
            } catch (e: Exception) {
                logger.warn("Failed to parse traceGlobals JSON: ${e.message}")
            }
        }
        if (eventGlobalsJson != null) {
            try {
                val eventGlobalsMap =
                    objectMapper.readValue(
                        eventGlobalsJson,
                ...
                eventGlobalsMap.forEach { (key, value) ->
                    log.eventGlobals.add(GlobalAttribute("event", mapOf(key to value)))
                }
            } catch (e: Exception) {
                logger.warn("Failed to parse eventGlobals JSON: ${e.message}")
            }
        }
        // Fallback to default globals if none stored
        if (log.traceGlobals.isEmpty() && log.eventGlobals.isEmpty()) {
    ...
    private fun extractTraceId(record: Map<String, Any?>): String? =
        when {
            ...
            record.containsKey("t_concept_name") -> {
                record["t_concept_name"]?.toString()
            }
    ...
    private fun buildTrace(
        ...
        // Build events
        val allEvents =
            traceRecords.mapNotNull { record ->
                buildEvent(record)
            }
     ...
     */
    private fun populateTraceAttributes(
        trace: Trace,
        record: Map<String, Any?>,
        traceId: String,
    ) {
        ...
        val traceData =
            if (hasProjected) {
                ...
                logger.debug(
                    "populateTraceAttributes - projected keys: {}, hasRealProjected: {}, record has 'trace': {}, trace type: {}",
                    projected.keys,
                    hasRealProjected,
                    record.containsKey("trace"),
                    record["trace"]?.javaClass?.name,
                )
        ...
        if (hasProjected) {
            ...
            if (traceData.isNotEmpty() && hasRealTraceData) {
                trace.conceptName =
                    if (traceData.containsKey("traceId")) {
    ...
    private fun copyAttributes(
        ...
        val internalFields = setOf("eventId", "createdAt", "traceId", "logId", "updatedAt") + INTERNAL_ALIAS_KEYS
