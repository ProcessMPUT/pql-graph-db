...
class Neo4jDataStoreRepository(
    ...
    override fun findById(id: String): DataStore? =
        driver.session().use { session ->
            session.executeRead { tx ->
                ...
                if (result.hasNext()) {
                    toDomain(result.single()["ds"].asNode())
                } else {
                    null
                }
            }
    ...
    override fun deleteWithLogs(id: String): Boolean {
        ...
        driver.session().use { session ->
            val params = mapOf<String, Any?>("dataStoreId" to id)
            deleteLogSubtreesBatched(
                session,
                "MATCH (:DataStore {dataStoreId: \$dataStoreId})-[:CONTAINS_LOG]->(l:Log)",
                params,
            )
            // Log and datastore nodes are few — one transaction is fine here.
            session.executeWrite { tx ->
                tx.run(
                    """
                    MATCH (ds:DataStore {dataStoreId: ${'$'}dataStoreId})
                    OPTIONAL MATCH (ds)-[:CONTAINS_LOG]->(log:Log)
                    DETACH DELETE log, ds
                    """.trimIndent(),
                    params,
                ).consume()
            }
