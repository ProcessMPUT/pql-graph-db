...
data class CypherQuery(
    ...
    /**
     * When set, HierarchyReconstructor sorts events within each trace by this field.
     * Used for hoisted GROUP BY queries where Neo4j ORDER BY on RETURN corrupts count values.
     * Example: "conceptName" for ORDER BY e:name, null for natural order.
     */
    val eventSortField: String? = null,
) {
    override fun toString(): String = "CypherQuery(query='$query', parameters=$parameters, hierarchicalLimits=$hierarchicalLimits)"
}
