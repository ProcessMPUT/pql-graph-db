...
class CypherCodegenTest {
    ...
    private fun stdAttr(scope: Scope, xesName: String, type: Type = Type.STRING) = ResolvedAttribute(
    ...
    private fun customAttr(scope: Scope, name: String, type: Type = Type.UNKNOWN) = ResolvedAttribute(
        name = name,
        baseScope = scope,
    ...
    private fun selectPlanFull(
        ...
        filter: com.processm.processminterpreter.domain.pql.resolved.ResolvedExpression? = null,
    ...
    fun `WHERE equality binds RHS to a parameter and references left attribute`() {
        val a = stdAttr(Scope.EVENT, "concept:name")
        val filter = TypedBinaryOp(
            op = BinaryOperator.EQ,
            left = a,
            right = TypedLiteral("Registration", Type.STRING, loc),
    ...
    fun `nested XES attribute equality compares string representations`() {
        val nested = customAttr(
            Scope.LOG,
            XesNestedAttributePath.encodedChildKey("meta_org:group_events_average", "Maternity ward"),
        )
        val filter = TypedBinaryOp(
            op = BinaryOperator.EQ,
            left = nested,
            right = TypedLiteral("0.016", Type.STRING, loc),
            type = Type.BOOLEAN,
            location = loc,
        )
    ...
    fun `candidate log probe keeps datastore scope and returns only distinct log ids`() {
        ...
        val filter = TypedBinaryOp(
            op = BinaryOperator.EQ,
            left = logName,
            right = TypedLiteral("JournalReview", Type.STRING, loc),
            type = Type.BOOLEAN,
            location = loc,
        )
        val q = codegen.generate(
    ...
    fun `candidate log probe preserves explicit log ordering and ignores deeper order keys`() {
        ...
        val q = codegen.generate(
            CandidateLogPlan(
                ...
                filter = null,
                orderBy = listOf(
                    OrderKey(logName, OrderDirection.DESC),
                    OrderKey(eventName, OrderDirection.ASC),
                ),
                location = loc,
            ),
        )
    ...
    fun `trace-only WHERE filters before event expansion`() {
        ...
        val filter = TypedBinaryOp(
            ...
            left = diagnosis,
            right = TypedLiteral(null, Type.UNKNOWN, loc),
            type = Type.BOOLEAN,
            location = loc,
        )
    ...
    fun `implicit full hierarchy trace-only WHERE does not expand events`() {
        ...
        assertTrue(q.cypher.indexOf("WHERE trace.Diagnosis IS NOT null") < q.cypher.indexOf("RETURN 1 AS _kind"), q.cypher)
    ...
    fun `AND OR compose predicates with parentheses`() {
        val a = stdAttr(Scope.EVENT, "concept:name")
        val left = TypedBinaryOp(BinaryOperator.EQ, a, TypedLiteral("A", Type.STRING, loc), Type.BOOLEAN, loc)
        val right = TypedBinaryOp(BinaryOperator.EQ, a, TypedLiteral("B", Type.STRING, loc), Type.BOOLEAN, loc)
        val or = TypedBinaryOp(BinaryOperator.OR, left, right, Type.BOOLEAN, loc)
        val q = codegen.generate(
    ...
    fun `bare event classifier GROUP BY returns grouped event map without original event node`() {
        ...
        assertTrue(c.contains("RETURN log, trace, {resource: _gb_0} AS event"), c)
    ...
    fun `bare event GROUP BY honors ordering by the grouped attribute`() {
        ...
        val q = codegen.generate(
            selectPlanFull(
                ...
                orderBy = listOf(OrderKey(eventName, OrderDirection.ASC)),
            ),
        )
    ...
    fun `bare event GROUP BY with hierarchical limits pushes caps before grouping the whole log`() {
        ...
        assertTrue(q.cypher.contains("RETURN log, trace, event, _event_order_0, _group_first_event_order_"), q.cypher)
    ...
    fun `scoped trace aggregation over log attribute preserves grouped event placeholder count`() {
        val logName = stdAttr(Scope.LOG, "concept:name")
        val traceMinLogName = Aggregation("min", logName, Type.STRING, loc, scope = Scope.TRACE)
    ...
    fun `empty projection aggregate ORDER BY returns placeholder event without exposing order aliases`() {
        ...
        val avgTotal = Aggregation("avg", total, Type.NUMBER, loc)

        val q = codegen.generate(
            selectPlanFull(
                columns = emptyList(),
                orderBy = listOf(OrderKey(avgTotal, OrderDirection.ASC)),
            ),
        )

        assertTrue(q.cypher.contains("avg(event.cost) AS _order_0"), q.cypher)
        assertTrue(q.cypher.contains("RETURN log, trace, {} AS event"), q.cypher)
    ...
    fun `log scoped hoisted aggregates preserve original trace event placeholder hierarchy`() {
        ...
        val avgTotal = Aggregation("avg", total, Type.NUMBER, loc, scope = Scope.LOG)
        val minTimestamp = Aggregation("min", timestamp, Type.DATETIME, loc, scope = Scope.LOG)
    ...
    fun `max aggregation on timestamp`() {
        ...
        val maxTs = Aggregation("max", ts, Type.DATETIME, loc)
        val q = codegen.generate(
            selectPlanFull(
    ...
    fun `hoisted event GROUP BY emits trace variant reconstruction query`() {
        ...
        val countTraceName = Aggregation("count", traceName, Type.NUMBER, loc)

        val q = codegen.generate(
            selectPlanFull(
                columns = listOf(ProjectedColumn(eventName, alias = "e_concept_name", scope = Scope.EVENT)),
                groupBy = GroupBySpec(keys = listOf(hoistedName), hasAggregation = true),
                orderBy = listOf(OrderKey(countTraceName, OrderDirection.DESC)),
            ),
        )
        ...
        assertTrue(c.contains("reduce(acc = '', value IN _trace_variant_"), c)
        assertTrue(c.contains("_trace_variant_key_ AS _trace_variant_"), c)
        assertEquals(Scope.TRACE, q.columnAliases["_trace_variant_"]!!.scope)
        assertTrue(q.columnAliases["_trace_variant_"]!!.synthetic)
    }
    ...
    fun `mixed trace and hoisted event GROUP BY respects trace scoped order tiebreakers`() {
        ...
        val q = codegen.generate(
            selectPlanFull(
                ...
                groupBy = GroupBySpec(keys = listOf(traceName, hoistedName), hasAggregation = true),
                orderBy = listOf(
                    OrderKey(countTraceName, OrderDirection.DESC),
                    OrderKey(traceName, OrderDirection.ASC),
                ),
            ),
        )
    ...
    fun `bare hoisted event GROUP BY projects grouped values as variant events`() {
        ...
        assertTrue(c.contains("_order_0 AS count_trace_concept_name"), c)
        ...
        assertEquals("count(trace:concept:name)", q.columnAliases["count_trace_concept_name"]!!.pqlExpression)
        assertEquals(Scope.TRACE, q.columnAliases["count_trace_concept_name"]!!.scope)
    }
    ...
    fun `hoisted event GROUP BY ordered by grouped event attribute sorts events before variant collection`() {
        ...
        val c = q.cypher
        assertTrue(
            c.contains("ORDER BY log.logId, trace.importOrder, event.activity ASC, event.importOrder"),
            "expected event attribute ordering before trace variant collection: $c",
        )
        assertTrue(
            c.contains("ORDER BY _trace_group_order_, _event_idx_"),
            "expected final variant ordering by ProcessM-style grouped trace order: $c",
        )
    }
    ...
    fun `hoisted event GROUP BY supports variant aggregate projection and order`() {
        ...
        val minTimestamp = Aggregation("min", eventTimestamp, Type.DATETIME, loc)
        val minHoistedTimestamp = Aggregation("min", hoistedTimestamp, Type.DATETIME, loc)
        ...
        val q = codegen.generate(
            selectPlanFull(
                ...
                orderBy = listOf(OrderKey(minHoistedTimestamp, OrderDirection.ASC)),
            ),
        )
    ...
    fun `hoisted classifier GROUP BY renders classifier keys as trace variant value`() {
        val classifier = ResolvedAttribute(
    ...
    @Test
    fun `temporal difference uses duration dot between`() {
        val ts = stdAttr(Scope.EVENT, "time:timestamp", Type.DATETIME)
        val maxTs = Aggregation("max", ts, Type.DATETIME, loc)
        val minTs = Aggregation("min", ts, Type.DATETIME, loc)
        val diff = TypedBinaryOp(BinaryOperator.MINUS, maxTs, minTs, Type.NUMBER, loc)
        val q = codegen.generate(
            selectPlanFull(
        ...
        assertTrue(q.cypher.contains("duration.between("), "expected duration.between in: ${q.cypher}")
    }
    ...
    fun `trace-scope GROUP BY uses group key as synthetic trace identity`() {
        ...
        val maxTs = Aggregation("max", hoistedTs, Type.DATETIME, loc)
        val minTs = Aggregation("min", hoistedTs, Type.DATETIME, loc)
        val duration = TypedBinaryOp(BinaryOperator.MINUS, maxTs, minTs, Type.NUMBER, loc)
        ...
        val q = codegen.generate(
            selectPlanFull(
                ...
                orderBy = listOf(OrderKey(duration, OrderDirection.DESC)),
                materializedScopes = setOf(Scope.LOG, Scope.TRACE, Scope.EVENT),
            ),
        )
        ...
        assertTrue(c.contains("duration.between(_cagg_1, _cagg_0) AS duration"), c)
    ...
    fun `compiled trace aggregate query keeps compact row shape under full materialization`() {
        ...
        assertTrue(c.contains("_gb_0 AS _trace_group_0"), c)
        assertTrue(c.contains("duration.between(_cagg_1, _cagg_0) AS col_0"), c)
        assertTrue(c.contains("ORDER BY duration.inSeconds(_cagg_1, _cagg_0).seconds DESC"), c)
    ...
    fun `sum null guard wraps sum in CASE WHEN count = 0 null ELSE sum`() {
        ...
        val sumCost = Aggregation("sum", cost, Type.NUMBER, loc)
        val q = codegen.generate(
            selectPlanFull(
    ...
    fun `simple ORDER BY emits ORDER BY clause with ASC`() {
        ...
        val q = codegen.generate(
            selectPlanFull(
                ...
                orderBy = listOf(OrderKey(ts, OrderDirection.ASC)),
            ),
        )
    ...
    fun `event ORDER BY preserves hierarchy before sorting events`() {
        ...
        val q = codegen.generate(
            selectPlanFull(
                columns = listOf(ProjectedColumn(name, alias = "n", scope = Scope.EVENT)),
                orderBy = listOf(OrderKey(name, OrderDirection.ASC)),
            ),
        )
    ...
    fun `mixed trace and event ORDER BY applies keys at their hierarchy scope`() {
        ...
        val q = codegen.generate(
            selectPlanFull(
                ...
                orderBy =
                    listOf(
                        OrderKey(traceTotal, OrderDirection.DESC),
                        OrderKey(eventTimestamp, OrderDirection.ASC),
                    ),
            ),
        )
    ...
    fun `aggregate ORDER BY references the SELECT alias`() {
        ...
        val q = codegen.generate(
            selectPlanFull(
                ...
                groupBy = GroupBySpec(keys = listOf(name), hasAggregation = true),
                orderBy = listOf(OrderKey(count, OrderDirection.DESC)),
            ),
        )
        val c = q.cypher
    ...
    fun `classifier ORDER BY emits IS NOT NULL filter`() {
        ...
        val q = codegen.generate(
            selectPlanFull(
                ...
                orderBy = listOf(OrderKey(classifier, OrderDirection.ASC)),
            ),
        )
        val c = q.cypher
    ...
    fun `node-shaped event order pushes event limit with requested order`() {
        ...
        val q = codegen.generate(
            selectPlanFull(
                columns = emptyList(),
                orderBy = listOf(OrderKey(timestamp, OrderDirection.ASC)),
                limits = HierarchicalLimits(log = 1, trace = 3, event = 5),
                dataStoreId = "store-42",
            ),
        )

        assertTrue(q.cypher.contains("WITH event ORDER BY event.timestamp ASC LIMIT \$eventLimit"), q.cypher)
        assertTrue(q.cypher.contains("RETURN log, trace, event ORDER BY log.logId, trace.importOrder, event.timestamp ASC"), q.cypher)
        assertEquals(1L, q.parameters["logLimit"])
    ...
    fun `node-shaped event order keeps trace binding when only log limit is pushed`() {
        ...
        assertTrue(q.cypher.contains("WITH trace, event ORDER BY event.timestamp ASC RETURN trace, event"), q.cypher)
        assertTrue(q.cypher.contains("RETURN log, trace, event ORDER BY log.logId, trace.importOrder, event.timestamp ASC"), q.cypher)
    ...
    fun `event filter is applied before pushed event and trace limits`() {
        ...
        assertTrue(q.cypher.indexOf("LIMIT \$eventLimit") < q.cypher.indexOf("LIMIT \$traceLimit"), q.cypher)
    }
    ...
    fun `DELETE FROM event emits DETACH DELETE`() {
        ...
        val filter = TypedBinaryOp(
            op = BinaryOperator.EQ,
            left = name,
            right = TypedLiteral("Trash", Type.STRING, loc),
            type = Type.BOOLEAN, location = loc,
        )
    ...
    fun `parameter names are unique per binding`() {
        val a = stdAttr(Scope.EVENT, "concept:name")
        val eq1 = TypedBinaryOp(BinaryOperator.EQ, a, TypedLiteral("A", Type.STRING, loc), Type.BOOLEAN, loc)
        val eq2 = TypedBinaryOp(BinaryOperator.EQ, a, TypedLiteral("B", Type.STRING, loc), Type.BOOLEAN, loc)
        val and = TypedBinaryOp(BinaryOperator.AND, eq1, eq2, Type.BOOLEAN, loc)
        val q = codegen.generate(
    ...
    fun `now function is bound once per generated query`() {
        val now = ScalarFunction("now", emptyList(), scope = Scope.LOG, type = Type.DATETIME, location = loc)
