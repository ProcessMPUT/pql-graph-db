...
class LogServiceTest {
    ...
    /** CRUD tests don't touch import — wire never-used import collaborators once. */
    private fun service(repo: LogRepository) = LogService(
        logs = repo,
        dataStores = NoopDataStoreRepository(),
        importer = XesLogImporter(Mockito.mock(XESLoader::class.java)),
        resources = ClasspathBundledLogResourceReader(),
    )

    private class NoopDataStoreRepository : DataStoreRepository {
    ...
    fun `get throws LogNotFoundException when missing`() {
        val useCase = service(InMemoryLogRepository())
    ...
    fun `delete returns false when the log is missing`() {
        assertFalse(service(InMemoryLogRepository()).delete("nope"))
    }
    ...
    fun `deleteWithData removes the log as well`() {
        val repo = InMemoryLogRepository()
        repo.save(sampleLog("a"))
        assertTrue(service(repo).deleteWithData("a"))
