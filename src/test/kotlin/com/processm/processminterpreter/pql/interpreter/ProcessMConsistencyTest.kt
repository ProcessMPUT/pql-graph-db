...
class ProcessMConsistencyTest : BaseInterpreterTest() {
    ...
    fun prepareData() {
        try {
            ...
            val logFile = File("src/main/resources/logs/Hospital_log.xes.gz")
            if (logFile.exists()) {
                ...
                println("WARNING: Local Hospital_log.xes.gz not found. Local tests will run on empty DB.")
            }
        } catch (e: Throwable) {
    ...
    fun compareWithRemote() {
        ...
        queries.forEach { (desc, query) ->
            ...
            print("LOCAL:  ")
            try {
                val localResult = executeDataStoreQuery(query)
