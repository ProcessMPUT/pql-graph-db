...
/**
 * End-to-end test of the ANTLR adapter: PQL text → PqlQuery.
...
class AntlrPqlParserTest {
    ...
    fun `empty SELECT expands to three star columns`() {
        val q = parser.parse("where e:name = 'A'") as PqlQuery.Select
    ...
    fun `explicit SELECT star also produces three star columns`() {
        val q = parser.parse("select *") as PqlQuery.Select
        assertEquals(3, q.columns.count { it.starAt != null })
    }

    @Test
    fun `scoped select all SELECT e star`() {
        val q = parser.parse("select e:*") as PqlQuery.Select
    ...
    fun `SELECT with mixed scoped star and attribute`() {
        val q = parser.parse("select t:*, e:name") as PqlQuery.Select
        assertEquals(2, q.columns.size)
        assertEquals(Scope.TRACE, q.columns[0].starAt)
        val attr = q.columns[1].expression as PqlExpression.AttributeRef
    ...
    fun `attribute reference preserves scope hint and name`() {
        val q = parser.parse("select e:timestamp") as PqlQuery.Select
        val a = q.columns[0].expression as PqlExpression.AttributeRef
    ...
    fun `hoisted attribute records hoisting level`() {
        val q = parser.parse("select ^e:name") as PqlQuery.Select
        val a = q.columns[0].expression as PqlExpression.AttributeRef
    ...
    fun `WHERE with equality produces BinaryOp EQ`() {
        val q = parser.parse("where e:name = 'Task A'") as PqlQuery.Select
        val bin = q.where as PqlExpression.Binary
        assertEquals(BinaryOperator.EQ, bin.op)
        assertEquals("name", (bin.left as PqlExpression.AttributeRef).name)
        assertEquals(PqlExpression.LiteralKind.STRING, (bin.right as PqlExpression.Literal).kind)
    }

    @Test
    fun `WHERE AND and OR compose correctly`() {
        val q = parser.parse("where e:name = 'A' and e:name = 'B' or e:name = 'C'") as PqlQuery.Select
        // AND has higher precedence than OR → top-level is OR
        val or = q.where as PqlExpression.Binary
        assertEquals(BinaryOperator.OR, or.op)
        val leftAnd = or.left as PqlExpression.Binary
        assertEquals(BinaryOperator.AND, leftAnd.op)
    }

    @Test
    fun `NOT wraps logic expression in unary`() {
        val q = parser.parse("where not e:name = 'A'") as PqlQuery.Select
        val not = q.where as PqlExpression.Unary
        assertEquals(UnaryOperator.NOT, not.op)
    }

    @Test
    fun `IS NULL becomes binary IS with null literal`() {
        val q = parser.parse("where [e:customX] is null") as PqlQuery.Select
        val bin = q.where as PqlExpression.Binary
        assertEquals(BinaryOperator.IS, bin.op)
        val lit = bin.right as PqlExpression.Literal
        assertEquals(PqlExpression.LiteralKind.NULL, lit.kind)
    }

    @Test
    fun `IS NOT NULL becomes binary IS_NOT with null literal`() {
        val q = parser.parse("where [e:customX] is not null") as PqlQuery.Select
        val bin = q.where as PqlExpression.Binary
        assertEquals(BinaryOperator.IS_NOT, bin.op)
    }

    @Test
    fun `IN list is lifted into InList on right`() {
        val q = parser.parse("where e:name in ('A', 'B', 'C')") as PqlQuery.Select
        val bin = q.where as PqlExpression.Binary
        assertEquals(BinaryOperator.IN, bin.op)
        val list = bin.right as PqlExpression.InList
        assertEquals(3, list.values.size)
        assertTrue(list.values.all { it is PqlExpression.Literal })
    }

    @Test
    fun `LIKE becomes binary LIKE with string RHS`() {
        val q = parser.parse("where e:name like 'Task%'") as PqlQuery.Select
        val bin = q.where as PqlExpression.Binary
    ...
    fun `arithmetic preserves operator precedence`() {
        val q = parser.parse("select 1 + 2 * 3") as PqlQuery.Select
        // + is top-level since * binds tighter
        val plus = q.columns[0].expression as PqlExpression.Binary
        assertEquals(BinaryOperator.PLUS, plus.op)
        val mul = plus.right as PqlExpression.Binary
    ...
    fun `parentheses do not appear as AST nodes`() {
        val q = parser.parse("select (1 + 2) * 3") as PqlQuery.Select
        val top = q.columns[0].expression as PqlExpression.Binary
        assertEquals(BinaryOperator.MUL, top.op)
        val left = top.left as PqlExpression.Binary
        assertEquals(BinaryOperator.PLUS, left.op)
    }

    @Test
    fun `aggregation function wraps ID into attribute ref argument`() {
        val q = parser.parse("select count(e:name)") as PqlQuery.Select
        val fn = q.columns[0].expression as PqlExpression.Call
        assertEquals("count", fn.name)
        assertEquals(1, fn.arguments.size)
        assertTrue(fn.arguments[0] is PqlExpression.AttributeRef)
    }

    @Test
    fun `scalar functions are captured with arguments`() {
        val q = parser.parse("select year(e:timestamp)") as PqlQuery.Select
        val fn = q.columns[0].expression as PqlExpression.Call
    ...
    fun `now has zero arguments`() {
        val q = parser.parse("select now()") as PqlQuery.Select
        val fn = q.columns[0].expression as PqlExpression.Call
    ...
    fun `GROUP BY produces list of attribute refs`() {
        val q = parser.parse("select e:name group by e:name") as PqlQuery.Select
        assertEquals(1, q.groupBy.size)
        assertEquals("name", (q.groupBy[0] as PqlExpression.AttributeRef).name)
    }

    @Test
    fun `ORDER BY preserves direction`() {
        val q = parser.parse("select e:name order by e:timestamp desc") as PqlQuery.Select
    ...
    fun `LIMIT with scoped numbers populates correct slots`() {
        val q = parser.parse("select e:name limit l:5, t:10, e:20") as PqlQuery.Select
    ...
    fun `DELETE defaults to EVENT when no scope given`() {
        val q = parser.parse("delete") as PqlQuery.Delete
    ...
    fun `DELETE with explicit scope and WHERE`() {
        val q = parser.parse("delete t where t:name = 'C1'") as PqlQuery.Delete
