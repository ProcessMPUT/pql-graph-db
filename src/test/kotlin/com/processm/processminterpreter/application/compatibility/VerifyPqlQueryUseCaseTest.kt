...
class VerifyPqlQueryUseCaseTest {
    ...
    fun `hoisted variant window mismatch is classified as nondeterministic when remote is in widened local result`() {
        ...
        val formatter = ProcessMJsonFormatter { projection ->
            formatterCalls++
            when (projection.logs.first().conceptName) {
        ...
        val result = VerifyPqlQueryUseCase(executeUseCase, remote, formatter).verify(
